...
```{r}
ny_noaa = separate(ny_noaa,date,c("year","month","day"))
...
ny_noaa %>%
  ...
  geom_point(data = ~ filter(.x, month %in% c("01", "07"))) +
  geom_line(data = ~ filter(.x, month %in% c("01", "07"))) +
...
The temperature in July is much higher than January, which indecates that New York is in the northern hemisphere.

```{r}
# What about the geo distribution?
stationinfo = distinct(select(ghcnd_stations(),c("id","latitude","longitude")))
stationRecord = ny_noaa %>%
  group_by(id,month) %>%
  summarise(mean_tmax = mean(tmax,na.rm = TRUE))%>%
  filter(!is.na(mean_tmax))
stationRecord = merge(stationRecord,stationinfo,by = "id",all = FALSE)
yearAverage = ggplot(stationRecord,aes(x = "longitude", y = "latitude")) +
  geom_point() +
  ylim(c(40,44))+
  xlim(c(-79,-72))
